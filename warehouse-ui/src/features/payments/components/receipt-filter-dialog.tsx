...
import { receiptTypeConfig, paymentStatusConfig } from '../payment.utils'
import { getDatePreset, DATE_PRESET_LABELS, type DatePreset } from '../payment-date-presets'
import { RECEIPT_FILTER_DEFAULT } from '../payment-filters'
import type { ReceiptFilterState } from '../payment-filters'
import type { ReceiptType, PaymentStatus } from '../types/payment.types'

interface ReceiptFilterDialogProps {
  open: boolean
  filter: ReceiptFilterState
  onApply: (filter: ReceiptFilterState) => void
  onClose: () => void
}

export function ReceiptFilterDialog({ open, filter, onApply, onClose }: ReceiptFilterDialogProps) {
  const [draft, setDraft] = useState<ReceiptFilterState>(filter)
  ...
  function handleReset() {
    setDraft(RECEIPT_FILTER_DEFAULT)
  }
        ...
        <div className="space-y-2">
          ...
          <div className="space-y-3">
            <div className="space-y-1.5">
              ...
              <Select
                value={draft.type}
                onValueChange={(v) => setDraft((d) => ({ ...d, type: v as ReceiptType | 'all' }))}
              >
